...
@app.route("/hshirt",methods=["GET","POST"])
def hshirt():
	try:
		conn=mysql.connect()
		cursor=conn.cursor(pymysql.cursors.DictCursor)
		...
		row = cursor.fetchall()
		return render_template("hshirt.html",row=row)
	except Exception as e:
		print(e)
	finally:
		cursor.close()
...
def lshirt(id):
	try:
		...
		return render_template("lshirt.html",row=row,user=user)
		...
		cursor.close()
		conn.close()
@app.route("/ashirt",methods=["GET","POST"])
def ashirt():
	try:
		conn=mysql.connect()
		cursor=conn.cursor(pymysql.cursors.DictCursor)
		cursor.execute("SELECT * FROM products WHERE Category='shirts'")
		row = cursor.fetchall()
		return render_template("ashirt.html",row=row)
	except Exception as e:
		print(e)
	finally:
		cursor.close()
